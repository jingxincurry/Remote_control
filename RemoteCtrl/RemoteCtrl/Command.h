...
protected:
    ...
    {   //获取当前系统的盘符信息
        std::string result;  //盘符列表
        for (int i = 1; i <= 26; i++) {  //A-Z盘符
		...
		lstPacket.push_back(CPacket(1, (BYTE*)result.c_str(), result.size())); //发送盘符列表到控制端
        return 0;
    }

    int MakeDirectoryInfo(std::list<CPacket>& lstPacket, CPacket& inPacket) {
        std::string strPath = inPacket.strData; //目录路径
        ...
        do {
            ...
            strncpy_s(finfo.szFileName, fdata.name, sizeof(finfo.szFileName) - 1); //文件名
            //listFileInfos.push_back(finfo);
            lstPacket.push_back(CPacket(2, (BYTE*)&finfo, sizeof(finfo))); //发送文件信息到控制端
    ...
    int RunFile(std::list<CPacket>& lstPacket, CPacket& inPacket) {
		...
		std::wstring widePath = AnsiPathToWide(strPath); //将ANSI路径转换为宽字符路径
		ShellExecuteW(NULL, NULL, widePath.c_str(), NULL, NULL, SW_SHOW); //使用ShellExecuteW函数运行文件
    ...
    int DownloadFile(std::list<CPacket>& lstPacket, CPacket& inPacket) {
        ...
        if (fp != NULL) {
			...
			data = _ftelli64(fp); //获取文件大小
			lstPacket.push_back(CPacket(4, (BYTE*)&data, 8)); //发送文件大小到控制端
			fseek(fp, 0, SEEK_SET); //移动文件指针回文件开头
            char buffer[1024];
            size_t nSize = 0;
            do {
    ...
    int MouseEvent(std::list<CPacket>& lstPacket, CPacket& inPacket) {
		MOUSEEV mouse; //鼠标事件结构体
		memcpy(&mouse, inPacket.strData.c_str(), sizeof(MOUSEEV)); //从输入包中提取鼠标事件信息
    ...
    int SendScreen(std::list<CPacket>& lstPacket, CPacket& inPacket) {
		...
		IStream* pStream = NULL; //创建一个内存流对象
