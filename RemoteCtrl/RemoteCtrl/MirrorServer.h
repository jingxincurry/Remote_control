...
template<MirrorOperator>class AcceptOverlapped;  //接收连接的重叠结构体，包含了OVERLAPPED结构体和其他相关信息，用于异步接受连接操作
typedef AcceptOverlapped<EAccept> ACCEPTOVERLAPPED;  //接收连接的重叠结构体类型定义
...
private:
	SOCKET m_sock;  //套接字
	DWORD m_received; //接收数据的字节数
	DWORD m_flags;  //接收数据的标志
	std::shared_ptr<ACCEPTOVERLAPPED> m_overlapped; //重叠结构体，包含了OVERLAPPED结构体和其他相关信息，用于异步操作
	std::shared_ptr<RECVOVERLAPPED> m_recv;   //接收重叠结构体，包含了OVERLAPPED结构体和其他相关信息，用于异步接收操作
	std::shared_ptr<SENDOVERLAPPED> m_send;   //发送重叠结构体，包含了OVERLAPPED结构体和其他相关信息，用于异步发送操作
...
public:
	MirrorServer(const std::string& ip = "0.0.0.0", short port = 9527) : m_pool(10) {
		m_hIOCP = INVALID_HANDLE_VALUE; //IOCP句柄
