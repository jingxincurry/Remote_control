...
public:
	...
	static bool IsAdmin() { //判断当前用户是否具有管理员权限
		HANDLE hToken = NULL;
	...
	**/
	...
	{//通过修改开机启动文件夹来实现开机启动
		TCHAR sPath[MAX_PATH] = _T("");
		GetModuleFileName(NULL, sPath, MAX_PATH); //获取当前程序的路径
	...
	static bool Init()
	{//用于带mfc命令行项目初始化（通用）
		HMODULE hModule = ::GetModuleHandle(nullptr); //获取当前模块的句柄
		if (hModule == nullptr) {
